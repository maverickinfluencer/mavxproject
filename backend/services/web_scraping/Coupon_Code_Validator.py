    ...
    if(s[0].find("fashor")>0):
        ...
        for x in range(6):
            try:
                ...
                print("hi")
        a_cart= driver.find_element_by_css_selector('.btn.btn--full.btn--status.btn--animated.js-product-button-add-to-cart')
        driver.execute_script("arguments[0].click();", a_cart)
        time.sleep(3)
        go_cart= driver.find_element_by_css_selector('.header__btn-cart.position-relative.d-flex.align-items-center.text-nowrap.js-popup-button')
        driver.execute_script("arguments[0].click();", go_cart)
        time.sleep(3)
